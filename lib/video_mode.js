...
class VideoMode {
  ...
  constructor(modeWidth, modeHeight, modeBitsPerPixel) {
    ...
    if (modeWidth === undefined) {
      ...
      throw new Error('modeHeight cannot be undefined while modeWidth is not undefined.');
    }

    if (modeBitsPerPixel === undefined) {
      this[RAW_VIDEO_MODE] = new _VideoMode(parseInt(modeWidth), parseInt(modeHeight));
    } else {
      this[RAW_VIDEO_MODE] = new _VideoMode(parseInt(modeWidth), parseInt(modeHeight), parseInt(modeBitsPerPixel));
    }
  }
